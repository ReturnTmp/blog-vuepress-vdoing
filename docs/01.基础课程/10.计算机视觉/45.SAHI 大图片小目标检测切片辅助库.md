...
> 注意：需要注意数据标注文件、图像、json 文件最好要放在目录下，下面我给出脚本命令范例

```bash
python ./yolo2coco.py -ap ../TVDSDataSet/labels -s ../TVDSDataSet/json --image-path ../TVDSDataSet/images
```
...
> 注意：本章为博主个人项目中使用的常用命令，便于自身查找，读者可酌情跳过本章节
